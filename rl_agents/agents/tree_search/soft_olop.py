...
class SoftOLOPAgent(AbstractTreeSearchAgent):
    """
        An agent that uses Open Loop Optimistic Planning to plan a sequence of actions in an MDP.
    """
    def make_planner(self):
        return SoftOLOP(self.env, self.config)


class SoftOLOP(AbstractPlanner):
    """
    ...
    def __init__(self, env, config=None):
        self.leaves = None
        self.env = env
        super(SoftOLOP, self).__init__(config)

    @classmethod
    def default_config(cls):
        cfg = super(SoftOLOP, cls).default_config()
        cfg.update(
            {
                "upper_bound":
                {
    ...
    def make_root(self):
        root = SoftOLOPNode(parent=None, planner=self)
        self.leaves = [root]
    ...
    def prebuild_tree(self, branching_factor):
        ...
        for _ in range(self.config["horizon"]):
            ...
            for leaf in self.leaves:
                super(SoftOLOPNode, leaf).expand(branching_factor)
    ...
    def allocate_budget(self):
        ...
        for episodes in range(1, int(self.config["budget"])):
            if episodes * SoftOLOP.horizon(episodes, self.config["gamma"]) > self.config["budget"]:
                self.config["episodes"] = max(episodes - 1, 1)
                self.config["horizon"] = SoftOLOP.horizon(self.config["episodes"], self.config["gamma"])
                break
        else:
    ...
    def compute_u_values(self, node, path):
        ...
        node.value = 0 #self.config["gamma"] ** (len(path) + 1) / (1 - self.config["gamma"])
        node_t = node
...
class SoftOLOPNode(Node):
    STOP_ON_ANY_TERMINAL_STATE = True

    def __init__(self, parent, planner):
        super(SoftOLOPNode, self).__init__(parent, planner)
    ...
    def selection_rule(self):
        ...
        values = Node.all_argmax([self.children[a].value for a in actions])
        return actions[max(values, key=(lambda i: self.children[actions[i]].get_value()))]
    ...
    def update(self, reward, done):
        ...
        if done or (self.parent and self.parent.done) and SoftOLOPNode.STOP_ON_ANY_TERMINAL_STATE:
            self.done = True
        if self.done:
            reward = 0
